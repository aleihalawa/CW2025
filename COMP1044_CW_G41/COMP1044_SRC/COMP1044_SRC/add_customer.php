...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($name) && !empty($company)) {
        if (!empty($email) && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $stmt = $conn->prepare("INSERT INTO customers (name, email, phone, address, company) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $name, $email, $phone, $address, $company);
...
<html lang="en">
<head>
    <title>Add Customer - ABB Robotics CRM</title>
    <meta charset="UTF-8" />
...
<body>
  <div class="banner">
    <div class="welcome">Add Customer</div>
    <div class="logo">
  ...
  <div class="container">
    ...
    <form method="POST">
        ...
        <label>Phone:</label>
        <input type="text" name="phone" pattern="\d+" title="Phone number must contain digits only" required>
