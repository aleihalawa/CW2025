...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($customer_id && $type && $description && $date) {
        $stmt = $conn->prepare("INSERT INTO interaction_history (customer_id, date, type, description) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("isss", $customer_id, $date, $type, $description);

        if ($stmt->execute()) {
            $success = "Interaction recorded!";
        } else {
...
<html lang="en">
<head>
    <title>Add Interaction - ABB Robotics CRM</title>
    <meta charset="UTF-8" />
...
<body>
  <div class="banner">
    <div class="welcome">Add Interaction</div>
    <div class="logo">
  ...
  <div class="container">
    ...
    <form method="POST">
        ...
        <div class="btn-row">
            <button type="submit" class="submit-btn">Add Interaction</button>
